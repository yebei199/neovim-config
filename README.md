...
仓库同时维护 `.gitignore` 与根目录 `ignore` 两套忽略规则，但职责并不相同：`.gitignore` 用于定义哪些文件不应进入 Git 版本控制，防止运行期产物、IDE 痕迹和本地实验文件污染提交历史；`ignore` 则服务于 `rg`、`fd` 以及依赖它们的 Neovim picker，用来屏蔽搜索时没有价值的目录和临时文件，减少噪音并提高定位效率。像 `nvim.log`、通用 `*.log` 这类运行日志需要同时出现在两处，这样既不会被误提交，也不会在日常搜索和文件选择时频繁干扰结果。
